...
class Permission extends BasePermission
{
    ...
    public function enum(): Attribute
    {
        return Cache::attribute(function () {
            $class = $this->platform
                ? Platforms::getNamespace($this->platform).'Enums\\Permissions'
                : Permissions::class;
    ...
    public function platformName(): Attribute
    {
        return new Attribute(
            fn () => $this->platform ? Platforms::find($this->platform)->getName() : ___('dictionary.jellybean')
        );
    }
    ...
    public static function toSelect(bool $include_all = false): array
    {
        ...
        $permissions = self::wherePlatform(Platforms::active()->pluck('StudlyName'))->orWhereNull('platform')->get();
        ...
        foreach ($permissions as $permission) {
            ...
            if ($include_all) {
                $add = true;
            } elseif (($is_tenant && in_array($display, [TenantDisplay::Both, TenantDisplay::Tenant], true))
                || (!$is_tenant && in_array($display, [TenantDisplay::Both, TenantDisplay::Central], true))
            ) {
                $add = true;
            }
