...
class Permission extends BasePermission
{
    ...
    public function enum(): Attribute
    {
        return Cache::attribute(function () {
            $class = $this->fuse ? Fuses::getNamespace($this->fuse).'Enums\\Permissions' : Permissions::class;
    ...
    public function fuseName(): Attribute
    {
        return new Attribute(fn () => $this->fuse ? Fuses::find($this->fuse)->getName() : ___('dictionary.jellybean'));
    }
    ...
    public static function toSelect(bool $include_all = false): array
    {
        ...
        foreach (self::whereFuse(Fuses::active()->pluck('StudlyName'))->orWhereNull('fuse')->get() as $permission) {
            $add     = false;
            ...
            $return[$permission->fuse_name][$permission->id] = [
