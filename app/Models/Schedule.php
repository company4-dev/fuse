...
class Schedule extends Model
{
    ...
    {
        return new Attribute(fn (): bool => $this->end_date === null && $this->end_after === null);
    }
    ...
    public function generate_next_date($start): false|string
    {
        ...
        $hour        = 60 * 60;
        $day         = $hour * 24;
        $day_of_week = (int) $carbon_start->format('N');
        $dow         = null;
        $next_time   = $start;
        $week        = match ($this->week) {
            1 => 'first',
            2 => 'second',
            3 => 'third',
            4 => 'fourth',
        ...
        $dow = match ($this->dow) {
            8       => 'day',
            9       => 'weekday',
            default => now()->parse('Sunday +'.$this->dow.' days')->format('l')
        };
        ...
        if ($this->schedule === 1) {
            ...
            if ($this->recurrence_type === 1) {
                ...
                $next_year = now()
                    ->parse(($carbon_start->format('Y') + $this->every).'-'.$this->month.'-'.$this->day)
                    ->timestamp;
                ...
                if ($this->dow < 10) {
                    ...
                    $next_saturday = now()
                        ->parse($week.' saturday'.$dow.' '.($carbon_start->format('Y') + $this->every).'-'.$this->month
                            .'-01')
                        ->timestamp;
                    $next_sunday = now()
                        ->parse($week.' sunday'.$dow.' '.($carbon_start->format('Y') + $this->every).'-'.$this->month
                            .'-01')
                        ->timestamp;
                    $this_saturday = now()
    ...
    public function replace(array $inputs, $assignee, $generates = null): void
    {
        $this->dates()->whereDate('date', '>', $inputs['start_date'])->delete();
        static::store($inputs, $assignee, $generates);
    }

    public function store(array $inputs, $assignee, $generates = null): void
    {
        ...
        if (isset($inputs['start_time'])) {
            $this->duration
                = strtotime($inputs['end_datetime']) - strtotime($this->start_date.'T'.$inputs['start_time']);
        } else {
            $this->duration = null;
        }
