...
{
    public function validate_attributes(array $field, array $required = [], array $defaults = []): void
    {
        $field = array_merge(
            [
                'description'  => null,
        ...
        if (!$field['is_component']) {
            foreach ($required as $attribute) {
                if (!array_key_exists($attribute, $field)) {
                    throw new Exception(___(
                        'errors.exceptions.components.input.attribute-required',
                        [
                            $attribute,
                            $field['label'],
                        ]
            ...
            foreach ($field as $attribute => $value) {
                if (in_array($attribute, $skips, true)) {
                    if ($attribute === 'wrap_abstract class ') {
                    ...
                    if ($attribute === 'value' && $field['type'] === 'switch') {
                        $this->name = (bool) $this->$attribute;
                    }
                } else {
                    throw new Exception(___(
                        'errors.exceptions.components.input.invalid-attribute',
    ...
    private function validate_attribute_accepts(array $field, Mime $value): Mime
    {
        if ($field['type'] !== 'file') {
            throw new Exception(___(
    ...
    private function validate_attribute_autocomplete($field, $value): string
    {
    ...
    public static function validate_attribute_can_add(array $field, $value): bool
    {
        if ($field['type'] !== 'file') {
            throw new Exception(___(
                'errors.exceptions.components.form.invalid-attribute-type',
    ...
    public static function validate_attribute_can_delete(array $field, $value): bool
    {
        if ($field['type'] !== 'file') {
            throw new Exception(___(
                'errors.exceptions.components.form.invalid-attribute-type',
    ...
    public static function validate_attribute_files(array $field, $value): array
    {
        if ($field['type'] !== 'file') {
            throw new Exception(___(
                'errors.exceptions.components.form.invalid-attribute-type',
    ...
    private function validate_attribute_max(array $field, $value): int|null|string
    {
        if (!in_array($field['type'], ['date', 'datetime', 'number', 'range', 'time'], true)) {
            throw new Exception(___(
                'errors.exceptions.components.form.invalid-attribute-type',
                [
                    'max',
        ...
        if (
            (
                (in_array($field['type'], ['number', 'range'], true) && !is_int($value))
                || (in_array($field['type'], ['date', 'datetime', 'time'], true) && !is_string($value))
            ) && $value !== null
        ) {
    ...
    private function validate_attribute_min(array $field, $value): int|null|string
    {
        if (!in_array($field['type'], ['date', 'datetime', 'number', 'range', 'time'], true)) {
            throw new Exception(___(
                'errors.exceptions.components.form.invalid-attribute-type',
    ...
    public static function validate_attribute_modifier(array $field, $value): string
    {
        if (in_array($field['type'], ['date', 'date-range'], true)) {
            $valid_modifiers = [
                'change',
                'live',
                null,
            ];
        } elseif (in_array($field['type'], ['checkbox', 'radio', 'select', 'switch'], true)) {
            $valid_modifiers = [
                'change',
                null,
            ];
        ...
        return in_array($value, $valid_modifiers, true)
            ? $value
            : throw new Exception(___(
                ...
                [
                    Formatters::implode(
                        '`, `',
                        '` '.___('dictionary.or').' `',
                        array_map(static fn ($item): string => $item ?? 'null', $valid_modifiers)
                    ),
                    $field['label'],
                ]
            ));
    ...
    private function validate_attribute_multiple(array $field, $value): bool
    {
        if (!is_bool($value)) {
    ...
    private function validate_attribute_options(array $field, array|Collection $value): array|Collection|null
    {
        if (!in_array($field['type'], ['checkbox', 'radio', 'select'], true)) {
            return null;
        }
    ...
    private function validate_attribute_required(array $field, $value): bool
    {
        if (!is_bool($value)) {
    ...
    private function validate_attribute_step(array $field, $value): float|string
    {
        if (!in_array($field['type'], ['date', 'datetime', 'number', 'range', 'time'], true)) {
            throw new Exception(___(
                'errors.exceptions.components.form.invalid-attribute-type',
    ...
    private function validate_attribute_wire(array $field, array $value): array
    {
        $return         = [];
        $supported_keys = [
            'loading',
        ...
        foreach ($value as $key => $val) {
            ...
            if (!in_array($test, $supported_keys, true)) {
                throw new Exception(___(
                    'errors.exceptions.components.form.invalid-wire-attribute',
                    [$test, $field['label']]
                ));
            }
    ...
    public static function validate_attribute_wrap_class($field, array|string $value): ?string
    {
        if (!is_array($value)) {
