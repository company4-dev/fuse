...
trait BaseModel
{
    ...
    public function scopeWhereFuse(Builder $query, array|null|string $fuse = null)
    {
        if ($fuse === null) {
            $query->whereNull('fuse');
        } elseif (is_array($fuse)) {
            $query->whereIn('fuse', $fuse);
        } else {
            $query->where('fuse', $fuse);
        }
    }
