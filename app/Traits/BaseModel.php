...
trait BaseModel
{
    ...
    {
        return Attribute::make(fn (): bool => (bool) $this->onboarding_progress)->shouldCache();
    }
    ...
    // Methods
    public function log(string $message, array|float|string $args = [])
    {
        return Activity::log($message, $this, array_filter(is_array($args) ? $args : [$args]));
    }
    ...
    public function next_hex_id(Model $model, string $column = 'id', int $length = 6): mixed
    {
        ...
        $get_hex_id = static fn () => strtoupper(str_pad(
            base_convert(random_int(1, 32 ** $length - 1), 10, 32),
            $length,
            0,
            STR_PAD_LEFT
        ));
    ...
    public function next_reference(string $reference, string $column = 'reference', ?string $separator = null): mixed
    {
        ...
        $get_reference = static function () use ($original_reference, &$increment, $separator): string {
    ...
    public function time_ago(string $column): string|false
    {
        if ($this->$column !== null) {
            if (gettype($this->$column) === 'object' && $this->$column::class === Carbon::class) {
    ...
    public function scopeByUser(Builder $query, int $user_id): void
    {
        $query->when(
            Cache::hasColumn($this->getTable(), 'user_id'),
            static fn ($query) => $query->where('user_id', $user_id)
        );
    }

    public function scopeChunked(Builder $query, callable $callback, ?int $chunk = null): bool
    {
        return $query->chunk($chunk ?? config('settings.lists.chunk-size'), $callback);
    }
    ...
    public function scopeLimited(Builder $query, $limit = null): void
    {
        $query->limit($limit ?? config('settings.lists.items-per-page'));
    }

    public function scopeOrdered(Builder $query, string $column = 'name', $order = 'asc')
    {
        $order = strtolower($order);
        $table = $this->getTable();
        ...
        return $query
            ->orderByRaw('CAST('.$table.'.'.$column.' AS UNSIGNED) '.$order)
            ->orderBy($table.'.'.$column, $order);
    }

    public function scopeSearch(Builder $query, ?string $term = null, ?array $fields = null): Builder
    {
        if (!$term) {
            return $query;
        }
        ...
        $query->where(static function ($query) use (&$fields, $term): void {
    ...
    public function scopeWhereMorph(Builder $query, string $column_prefix, string $type, int|string $id): void
    {
        $query->where([
    ...
    public function scopeWherePlatform(Builder $query, array|null|string $platform = null): void
    {
        if ($platform === null) {
    ...
    public function save(array $options = [])
    {
        ...
        foreach ($this->attributes as $field => $value) {
            if (in_array($field, ['created_at_formatted', 'deleted_at_formatted', 'updated_at_formatted'], true)
