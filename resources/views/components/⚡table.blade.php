...
use App\Exports\Table as ExportsTable;
...
new class extends Component
{
    ...
    public function getTableData(bool $for_export = false): array
    {
        $return = null;
        $table  = new $this->table_class($this->id, $this->data);

        $return['columns'] = $table->getHeaders();

        if ($for_export) {
            $query = $table->getQuery($this);
        ...
        return array_merge(
            $return,
            [
                'has_actions' => $table->hasActions($this),
    ...
    public function mount(?string $table = null, ?int $id = null, ?array $data = null, bool $manual = false): void
    {
        $this->data   = $data;
        $this->id     = $id;
        $this->manual = $manual;
    ...
    public function submit($button): void
    {
        $this->form->process(
            $this,
            function () use ($button): void {
                if ($button === 'filter.export') {
                    new ExportsTable(Auth::user(), $this->getTableData(true));

                    Flux::toast(___('messages.features.exports.exporting'));
                }
            },
        );
    }
...
<div>
    ...
    @else
        ...
        @else
            <flux:table :paginate="$table['models']" pagination:scroll-to>
                <flux:table.columns>
                    @foreach ($table['columns'] as $column_name => $details)
                        <flux:table.column>{{ ___($column_name) }}</flux:table.column>
                    @endforeach
                                ...
                                <flux:table.cell>
                                    ...
                                    @elseif (array_key_exists('columns', $details))
                                        @foreach ((array) $details['columns'] as $column)
                                            ...
                                            @elseif (str_contains($column, '.'))
                                                @php([$relation, $column] = explode('.', $column))

                                                {!! $model->{$relation}->{$column} !!}
                                            @else
                                                {!! $model->{$column} !!}
                                            @endif
                                        @endforeach
                                    @elseif (isset($details['value']))
                                        ...
                                        @elseif (str_contains($details['value'], '.'))
                                            @php([$relation, $column] = explode('.', $details['value']))
