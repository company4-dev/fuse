...
<x-layouts.base class="min-h-screen bg-white dark:bg-zinc-800" :title="implode(' > ', array_map('___', $breadcrumbs ?? ['dictionary.dashboard']))">
        ...
        <flux:sidebar.nav>
            @foreach (Menu::get() as $link)
                ...
                @else
                    ...
                    @elseif (array_key_exists('modal', $link))
                        <flux:modal.trigger :name="$link['modal']">
                            <flux:sidebar.item
                                :badge="$link['count'] ?? null"
                                :icon="$link['icon']"
                                :wire:ref="$link['modal']"
        ...
        @isdev
            <flux:sidebar.nav>
                <flux:sidebar.group :heading="___('dictionary.jellybean')" class="grid">
                    <flux:sidebar.item
                        :href="route('developer.testing')"
                        icon="vial"
                        wire:navigate
                    >
                        {{ ___('developer.developer-testing') }}
                    </flux:sidebar.item>

                    {{-- <flux:sidebar.item
                        icon="toolbox"
                        x-on:click="$flux.modal('developer').show();"
                    >
                        {{ ___('developer.developer-tools') }}
                    </flux:sidebar.item> --}}
    ...
    <flux:main>
        <div class="flex items-center mb-3">
            ...
            @else
                <x-icon class="rounded-md size-12 bg-zinc-50 dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-700 p-2" :icon="$avatar" />
            @endif
