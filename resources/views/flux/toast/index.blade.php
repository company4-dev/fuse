...
<ui-toast
    x-data="{
        handleToast(event) {
            if (!this.$el.closest('ui-toast-group')) {
                this.$el.showToast(event.detail);
            }
        }
    }"
    ...
    <template>
        <div
            ...
            x-data="{
                ...
                clickable: false,
                init() {
                    // Get URL from data attribute set by Flux
                    this.url = this.$el.getAttribute('data-position');
                    this.clickable = !!this.url && this.url !== '';
                }
            }"
            :class="{ 'cursor-pointer hover:scale-[1.02] transition-transform': clickable }"
            @click="if (clickable && url) {
