...
3. `array|null $menu`<br>A `2D array` of menu items with the following keys:
   ...
   3. `array|string route`<br>The route for the menu item. This can be a string for a simple route like `users`.<br><br>For routes with parameters, if there is only one, this can be a passed as is (E.g. `[users.edit, $user->id]`), for anything else pass an array (E.g. `[users.edit-address, [$user->id, $user->address_id]]`)
